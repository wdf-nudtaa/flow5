...
void ObjectTreeView::readWingSectionTree(WingXfl *pWing, QModelIndex indexLevel)
{
    ...
    do
    {
        ...
        else if (field.compare("offset", Qt::CaseInsensitive)==0)               pWS.setOffset(dataIndex.data().toDouble()/Units::mtoUnit());
...
/*        if(m_pPlane->wing(m_enumActiveWingType)==pWing)
        ...
        for(int iws=0; iws<pWing->nSections(); iws++)
        {
            ...
            {
                dataItem = xfl::prepareDoubleRow("", "span position", wingsec.yPosition()*Units::mtoUnit(), QUnits::lengthUnitLabel());
                sectionFolder.front()->appendRow(dataItem);

                dataItem = xfl::prepareDoubleRow("", "chord", wingsec.chord()*Units::mtoUnit(), QUnits::lengthUnitLabel());
                sectionFolder.front()->appendRow(dataItem);
...
{
    QList<QStandardItem*> dataItem = xfl::prepareDoubleRow( "", "Volume mass", inertia.structuralMass()*Units::kgtoUnit(), QUnits::massUnitLabel());
    ...
    for(int iwm=0; iwm<inertia.pointMassCount(); iwm++)
    {
        ...
        pWingInertiaFolder->appendRow(wingPointMassFolder);
        {
            ...
            dataItem = xfl::prepareDoubleRow("", "mass", pm.mass()*Units::kgtoUnit(), QUnits::massUnitLabel());
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.front()->appendRow(dataItem);

            dataItem = xfl::prepareDoubleRow("", "x", pm.position().x*Units::mtoUnit(), QUnits::lengthUnitLabel());
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.front()->appendRow(dataItem);

            dataItem = xfl::prepareDoubleRow("", "y", pm.position().y*Units::mtoUnit(), QUnits::lengthUnitLabel());;
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.front()->appendRow(dataItem);

            dataItem = xfl::prepareDoubleRow("", "z", pm.position().z*Units::mtoUnit(), QUnits::lengthUnitLabel());
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.front()->appendRow(dataItem);
        }
    }
