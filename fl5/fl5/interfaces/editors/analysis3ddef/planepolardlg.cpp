    ...
    m_pfrInertia= new QFrame;
    {
        QVBoxLayout *pInertiaPageLayout = new QVBoxLayout;
        {
            ...
            QGridLayout *pInertiaDataLayout = new QGridLayout;
            {
                ...
                m_pdeXCoG = new FloatEdit;
                m_pdeZCoG = new FloatEdit;
                m_pdeIxx  = new FloatEdit;
                m_pdeIyy  = new FloatEdit;
                m_pdeIzz  = new FloatEdit;
                m_pdeIxz  = new FloatEdit;
                pInertiaDataLayout->addWidget(m_pfePlaneMass,1,2);
                pInertiaDataLayout->addWidget(m_pdeXCoG,     2,2);
                pInertiaDataLayout->addWidget(m_pdeZCoG,     3,2);
                pInertiaDataLayout->addWidget(m_pdeIxx,      4,2);
                pInertiaDataLayout->addWidget(m_pdeIyy,      5,2);
                pInertiaDataLayout->addWidget(m_pdeIzz,      6,2);
                pInertiaDataLayout->addWidget(m_pdeIxz,      7,2);

                pInertiaDataLayout->addWidget(new QLabel(QUnits::massUnitLabel()),    1,3);
                pInertiaDataLayout->addWidget(new QLabel(QUnits::lengthUnitLabel()),  2,3);
                pInertiaDataLayout->addWidget(new QLabel(QUnits::lengthUnitLabel()),  3,3);
                pInertiaDataLayout->addWidget(new QLabel(QUnits::inertiaUnitLabel()), 4,3);
                pInertiaDataLayout->addWidget(new QLabel(QUnits::inertiaUnitLabel()), 5,3);
                pInertiaDataLayout->addWidget(new QLabel(QUnits::inertiaUnitLabel()), 6,3);
                pInertiaDataLayout->addWidget(new QLabel(QUnits::inertiaUnitLabel()), 7,3);
...
void PlanePolarDlg::initPolar3dDlg(const Plane *pPlane, PlanePolar const *pWPolar)
{
    ...
    if((m_pPlane && m_pPlane->hasFuse()) || !m_pPlane)
    {
        ...
        if(pPlaneXfl && pPlaneXfl->fuseAt(0))
        {
            ...
            m_plabFuseWettedArea->setText("Wetted Area (WA) = "+strong+" "+QUnits::areaUnitLabel());
...
void PlanePolarDlg::enableControls()
{
    if(m_pPlane && !m_pPlane->isXflType())
    {
        ...
        m_pchViscAnalysis->setEnabled(false);
        m_prbViscFromCl->setEnabled(false);
        m_prbViscFromAlpha->setEnabled(false);
    }
...
void PlanePolarDlg::fillInertiaPage()
{
    ...
    m_pdeIxx->setValue(s_WPolar.Ixx()*Units::kgm2toUnit());
    m_pdeIyy->setValue(s_WPolar.Iyy()*Units::kgm2toUnit());
    m_pdeIzz->setValue(s_WPolar.Izz()*Units::kgm2toUnit());
    m_pdeIxz->setValue(s_WPolar.Ixz()*Units::kgm2toUnit());
}
...
void PlanePolarDlg::readInertiaData()
{
    ...
    else
    {
        ...
        s_WPolar.setCoGx(m_pdeXCoG->value()/Units::mtoUnit());
        s_WPolar.setCoGz(m_pdeZCoG->value()/Units::mtoUnit());
        s_WPolar.setIxx(m_pdeIxx->value()/Units::kgm2toUnit());
        s_WPolar.setIyy(m_pdeIyy->value()/Units::kgm2toUnit());
        s_WPolar.setIzz(m_pdeIzz->value()/Units::kgm2toUnit());
        s_WPolar.setIxz(m_pdeIxz->value()/Units::kgm2toUnit());
    }
}
