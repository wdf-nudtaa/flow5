...
void FuseFrameWt::drawScaleLegend(QPainter &painter)
{
    ...
    painter.drawText(5,3*DisplayOptions::textFontStruct().height(), QString("x  = %1").arg(m_CursorPos.x() * Units::mtoUnit(),7,'f',2) + QUnits::lengthUnitLabel());
    painter.drawText(5,4*DisplayOptions::textFontStruct().height(), QString("y  = %1").arg(m_CursorPos.y() * Units::mtoUnit(),7,'f',2) + QUnits::lengthUnitLabel());
    painter.restore();
}
