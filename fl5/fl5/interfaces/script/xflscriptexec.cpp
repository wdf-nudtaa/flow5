...
bool XflScriptExec::makeFoils()
{
    ...
    for(int ifo=0; ifo<m_pScriptReader->m_FoilDatList.count(); ifo++)
    {
        ...
        {
            Foil *pFoil = new Foil();
            objects::readFoilFile(datPathName.toStdString(), pFoil);
            if(pFoil)
            {
...
//        pXFoilTask->setEventDestination(this);
        ...
        pXFoilTask->initialize(pAnalysis, false, true, true);

        m_nTaskStarted++;
        strong = "Starting "+QString::fromStdString(pAnalysis->pFoil->name())+" / "+QString::fromStdString(pAnalysis->pPolar->name())+"\n";
        traceLog(strong);
