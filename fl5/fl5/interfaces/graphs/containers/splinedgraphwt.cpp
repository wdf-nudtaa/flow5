...
void SplinedGraphWt::paintEvent(QPaintEvent* pEvent)
{
    ...
    {
        drawSpline(    &m_Spline, painter, m_pGraph->xScale(), -m_pGraph->yScale(0), m_pGraph->offset(0));
        drawCtrlPoints(&m_Spline, painter, m_pGraph->xScale(), -m_pGraph->yScale(0), m_pGraph->offset(0), DisplayOptions::backgroundColor());
        if(m_Spline.bShowNormals())
            drawNormals(&m_Spline, painter, m_pGraph->xScale(), -m_pGraph->yScale(0)/m_pGraph->xScale(), m_pGraph->offset(0));
    }
