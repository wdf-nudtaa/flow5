...
bool OpPointWt::s_bNeutralLine(true);
...
void OpPointWt::paintOpPoint(QPainter &painter)
{
    ...
    drawFoil(painter, pFoil, -Alpha, 0.0, m_fScale, m_fScale*m_fScaleY, m_ptOffset, s_bFillFoil, clr);

    if(pFoil->pointStyle()>0)
        drawFoilPoints(painter, pFoil, -Alpha, m_fScale,m_fScale*m_fScaleY, m_ptOffset,
...
{
    s_bNeutralLine = !s_bNeutralLine;
    s_pXDirect->checkNeutralLineAction(s_bNeutralLine);
    update();
}
...
void OpPointWt::setNeutralLineColor(QColor const&clr) {s_NeutralStyle.m_Color = xfl::tofl5Clr(clr);}
