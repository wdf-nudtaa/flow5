...
void PlaneAnalysisDlg::storePOpps()
{
    if(PlaneOpp::bStoreOpps3d())
    {
        m_pLastPOpp = s_pXPlane->storePOpps(m_pActiveTask->planeOppList());
    }
    else
    {
        m_pActiveTask->clearPOppList();
        m_pLastPOpp = nullptr;
    }
}
...
PlaneTask* PlaneAnalysisDlg::analyze(Plane *pPlane, PlanePolar *pWPolar, std::vector<double> const &opplist, std::vector<T8Opp> const &ranges)
{
    if(pPlane->name().compare(pWPolar->planeName())!=0)
    {
        return nullptr;
    }
...
/*    m_pActiveTask->setEventDestination(this);
    connect(m_pActiveTask,                  &Task3d::outputMessage,    this, &PlaneAnalysisDlg::onOutputMessage);
    connect(m_pActiveTask,                  &Task3d::taskFinished,     this, &PlaneAnalysisDlg::onTaskFinished);*/
    m_pActiveTask->setAnalysisStatus(xfl::RUNNING);

    m_pActiveTask->setObjects(pPlane, pWPolar);

    if(pWPolar->isType123() || pWPolar->isType5()) m_pActiveTask->setOppList(opplist);
    else if(pWPolar->isType6())                    m_pActiveTask->setCtrlOppList(opplist);
    else if(pWPolar->isType7())                    m_pActiveTask->setStabOppList(opplist);
    else if(pWPolar->isType8())                    m_pActiveTask->setT8OppList(ranges);
