...
void XSailActions::makeActions()
{
    ...
    {
        m_p3dView= new QAction(QIcon(":/sailimages/sailboat_512.png"), tr("3d"), m_pXSail);
        m_p3dView->setStatusTip(tr("Switch to the 3d view"));
        m_p3dView->setShortcut(Qt::Key_F4);
        m_p3dView->setCheckable(true);
        connect(m_p3dView,        SIGNAL(triggered()), m_pXSail, SLOT(on3dView()));

        m_pPolarView = new QAction(QIcon(":/icons/OnPolarView.png"), tr("Polars"), m_pXSail);
        m_pPolarView->setStatusTip(tr("Switch to the display of polar graphs"));
    ...
    m_p3dLightAct = new  QAction(QIcon(":/icons/light.png"), tr("Light settings"), m_pXSail);
    ...
    m_pImportAnalysisFromXml = new QAction(tr("Import from xml"), m_pXSail);
    connect(m_pImportAnalysisFromXml, SIGNAL(triggered()), m_pXSail, SLOT(onImportBtPolarFromXML()));

    m_pShowAnalysisWindow = new QAction(tr("Open the analysis window"), m_pXSail);
    ...
    m_pAnalysis3dSettings = new QAction(tr("3d analysis settings"), m_pXSail);
    connect(m_pAnalysis3dSettings, SIGNAL(triggered()), m_pXSail->s_pMainFrame, SLOT(on3dAnalysisSettings()));

    m_pMeshInfo = new QAction(tr("Mesh information"), m_pXSail);
    connect(m_pMeshInfo, SIGNAL(triggered()), m_pXSail, SLOT(onMeshInfo()));

    m_pConnectTriangles = new QAction(tr("Make triangle connections"), m_pXSail);
    ...
    m_pCheckPanels = new QAction(tr("Check panels"), m_pXSail);
    connect(m_pCheckPanels, SIGNAL(triggered()), m_pXSail, SLOT(onCheckPanels()));

    m_pShowNormals = new QAction(tr("Show panel normals"), m_pXSail);
...
{
    m_pDefineBoatAct = new QAction(tr("Define a new boat"), m_pXSail);
    m_pDefineBoatAct->setStatusTip(tr("Shows a dialogbox to create a new boat definition"));
    m_pDefineBoatAct->setShortcut(Qt::Key_F3);
    connect(m_pDefineBoatAct, SIGNAL(triggered()), m_pXSail, SLOT(onNewBoat()));

    m_pEditBoatAct = new QAction(tr("Edit"), m_pXSail);
    m_pEditBoatAct->setStatusTip(tr("Shows a form to edit the currently selected boat"));
    ...
    m_pRenameCurBoatAct = new QAction(tr("Rename"), m_pXSail);
    ...
    m_pDuplicateCurBoat = new QAction(tr("Duplicate"), m_pXSail);
    ...
    m_pShowBoatBtOpps = new QAction(tr("Show"), m_pXSail);
    connect(m_pShowBoatBtOpps,   SIGNAL(triggered()), m_pXSail, SLOT(onShowBtOpps()));

    m_pHideBoatBtOpps = new QAction(tr("Hide"), m_pXSail);
    connect(m_pHideBoatBtOpps,   SIGNAL(triggered()), m_pXSail, SLOT(onHideBtOpps()));

    m_pDeleteBoatBtOpps = new QAction(tr("Delete all"), m_pXSail);
...
{
    m_pDefineBtPolar = new QAction(tr("Define an analysis"), m_pXSail);
    m_pDefineBtPolar->setShortcut(Qt::Key_F6);
    m_pDefineBtPolar->setStatusTip(tr("Define an analysis for the current boat"));
    connect(m_pDefineBtPolar, SIGNAL(triggered()), m_pXSail, SLOT(onDefinePolar()));

    m_pDuplicateCurBtPolar = new QAction(tr("Duplicate"), m_pXSail);
    ...
    m_pDuplicateBtPolar = new QAction(tr("Duplicate an existing analysis"), m_pXSail);
    ...
    m_pEditBtPolar = new QAction(tr("Edit"), m_pXSail);
    m_pEditBtPolar->setStatusTip(tr("Modify the analysis parameters of this polar"));
    ...
    m_pBtPolarNameAct = new QAction(tr("Rename current"), m_pXSail);
...
{
    m_pEditMainSail = new QAction(tr("Edit"), m_pXSail);
