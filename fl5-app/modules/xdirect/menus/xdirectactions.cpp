...
//    connect(m_pNormalizeFoil, SIGNAL(triggered()), m_pXDirect, SLOT(onNormalizeFoil()));
    ...
    m_pFillFoil = new QAction(QIcon(":/icons/fillfoil.png"),"Fill selected", this);
    m_pFillFoil->setToolTip("<p>Fill the interior of the active foil</p>");
    m_pFillFoil->setCheckable(true);
    connect(m_pFillFoil, SIGNAL(triggered(bool)), m_pXDirect, SLOT(onFillFoil(bool)));

    m_pShowTEHinge = new QAction(QIcon(":/icons/TEHinge.png"), "Show T.E. hinge", this);
    m_pShowTEHinge->setToolTip("<p>Show the position of the trailing edge flap's hinge</p>");
    m_pShowTEHinge->setCheckable(true);
    connect(m_pShowTEHinge, SIGNAL(triggered(bool)), m_pXDirect, SLOT(onShowTEHinge(bool)));

    m_pShowLEPosition = new QAction(QIcon(":/icons/showLE.png"), "Show L.E. position", this);
    m_pShowLEPosition->setToolTip("<p>Show the position of the leading edge</p>");
