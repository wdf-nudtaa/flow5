...
QVector<AnalysisRange> *AnalysisRangeTable::activeRange() const
{
    ...
    else
    {
        switch(m_PolarType)
        {
            ...
            case xfl::T4POLAR:     return &s_T4Range;
...
void AnalysisRangeTable::loadSettings(QSettings &settings)
{
    ...
    settings.beginGroup("AnalysisRangeTable");
    {
        ...
        nRange = settings.value("NT4Range", s_T4Range.size()).toInt();
        if(nRange>0)
        {
            s_T4Range.resize(nRange);
            for(int i=0; i<nRange; i++)
            {
        ...
        else
        {
            // force one line
            s_T4Range.resize(1);
            s_T4Range.front() = {true, 0.0, 1.0, 0.5};
        }
...
void AnalysisRangeTable::saveSettings(QSettings &settings)
{
    settings.beginGroup("AnalysisRangeTable");
    {
        ...
        settings.setValue("NT4Range", s_T4Range.size());
