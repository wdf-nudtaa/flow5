...
void PanelAnalysisTest::onMakeGraph()
{
    ...
    m_pGraph->setXVariableList({"z ("+Units::lengthUnitQLabel()+")"});
    m_pGraph->setYVariableList({"phi", "V ("+Units::speedUnitQLabel()+")"});
    ...
    else if(m_prbVelocity->isChecked())
    {
        for(int i=0; i<curvecount; i++)
        {
            pCurveV[i] = m_pGraph->addCurve("V ("+Units::speedUnitQLabel()+") "+QString::fromStdString(m_pOpp3d->polarName()+m_pOpp3d->title(false)));
