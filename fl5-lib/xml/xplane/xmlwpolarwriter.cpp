...
void XmlWPolarWriter::writeXMLWPolar(const PlanePolar *pWPolar)
{
    if(!pWPolar) return;
...
void XmlWPolarWriter::writeWPolarData(PlanePolar const *pWPolar)
{
...
//    PlaneXfl const *pPlaneXfl = dynamic_cast<PlaneXfl const*>(pPlane);
    ...
    writeStartElement("Polar");
    {
        ...
        writeTextElement("Type",                 xml::polarType(pWPolar->type()));
        writeComment("Available methods are: LLT, VLM1, VLM2, QUADS, TRIUNIFORM, TRILINEAR");
        writeTextElement("Method",               xml::analysisMethod(pWPolar->analysisMethod()));
        ...
        writeStartElement("Reference_Dimensions");
        {
            ...
            writeTextElement("Reference_Dimensions",    xml::referenceDimension(pWPolar->referenceDim()));
        ...
        writeStartElement("Fuselage_Drag");
        {
            ...
            if(pWPolar->fuseDragMethod()==PlanePolar::KARMANSCHOENHERR) writeTextElement("Friction_Drag_Method", "Karman-Schoenherr");
        ...
        if(pWPolar->isControlPolar())
        {
            ...
            writeStartElement("Angle_range");
            {
                for(int iw=0; iw<int(pWPolar->m_AngleRange.size()); iw++)
                {
                    ...
                    {
                        for(uint c=0; c<pWPolar->m_AngleRange.at(iw).size(); c++)
                        {
                            QString str;
...
void XmlWPolarWriter::writeWPolarInertia(PlanePolar const*pWPolar)
{
    QString strange;
