...
void XflXmlWriter::writeNURBS(NURBSSurface const &nurbs, QVector<int> const &xPanels, bool bFramesOnly)
{
...
void XflXmlWriter::writeExtraDrag(QVector<ExtraDrag> const &XDrag)
{
    writeStartElement("ExtraDrag");
    {
        writeComment("Tag,  Area,  coef");
        for(int iex=0; iex<XDrag.size(); iex++)
        {
            if(fabs(XDrag.at(iex).area())>PRECISION && fabs(XDrag.at(iex).coef())>PRECISION)
            {
                ...
                strong  = XDrag.at(iex).tag() + strong;
...
void XflXmlWriter::writeWing(WingXfl const &wing, Vector3d WingLE, double Rx, double Ry, bool bWriteFoils)
{
    writeStartElement("wing");
    {
        ...
        writeTextElement("Type", WingXfl::type(wing.type()));
    ...
    for(int is=0; is<wing.nSections(); is++)
    {
        ...
        if(!foilnames.contains(ws.leftFoilName()))  foilnames.append(ws.leftFoilName());
        if(!foilnames.contains(ws.rightFoilName())) foilnames.append(ws.rightFoilName());
    }

    for(int is=0; is<foilnames.size(); is++)
    {
        Foil const*m_pFoil = Objects2d::foil(foilnames.at(is));
        if(!m_pFoil) continue;
