...
//        void cartesianCoords(double *g, Vector3d &ptGlobal) const;
        ...
        Vector3d const &nodeNormal(int iNode) const {assert(iNode>=0 && iNode<3); return m_S[iNode].normal();}
        Vector3d &nodeNormal(int iNode) {assert(iNode>=0 && iNode<3); return m_S[iNode].normal();}
        ...
        inline void splitAtEdgeMidPoints(std::vector<Triangle3d> &splittriangles) const;
        inline void splitAtCoG(std::vector<Triangle3d> &splittriangles) const;

        void splitIn4Triangles(std::vector<Triangle3d> &trianglelist) const;
        void splitIn3Triangles(Vector3d const &ptinside, std::vector<Triangle3d> &trianglelist) const;
...
inline void Triangle3d::splitAtEdgeMidPoints(std::vector<Triangle3d> &splittriangles) const
{
...
inline void Triangle3d::splitAtCoG(std::vector<Triangle3d> &splittriangles) const
{
    splittriangles.clear();
    splittriangles.push_back({m_S[0], m_CoG_g, m_S[1]});
    splittriangles.push_back({m_S[1], m_CoG_g, m_S[2]});
    splittriangles.push_back({m_S[2], m_CoG_g, m_S[0]});
}
