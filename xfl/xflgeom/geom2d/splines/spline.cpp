...
Spline::Spline()
{
    m_theStyle = LineStyle(true, Line::SOLID, 2, Qt::red, Line::NOSYMBOL);
...
{
    m_CtrlPt.clear();
    for(int ic=0; ic<spline.m_CtrlPt.size(); ic++)
    {
...
*/
void Spline::exportSpline(QTextStream &out) const
{
    QString strOut;

    for (int k=int(outputSize()-1);k>=0; k--)
    {
        strOut = QString::asprintf(" %11.5f  %11.5f\n",m_Output.at(k).x, m_Output.at(k).y);
        out << strOut;
    }
}


void Spline::exportToFile(QTextStream &out) const
{
    QString strOut;
    for (int k=0;k<outputSize(); k++)
    {
        strOut = QString::asprintf(" %13.5f  %13.5f\n", m_Output.at(k).x, m_Output.at(k).y);
        out << strOut;
    }
}
...
void Spline::setClosed(bool bClosed)
{
    ...
    if(bClosed)
    {
        if(m_CtrlPt.size()>0) m_CtrlPt.last() = m_CtrlPt.first();
        updateSpline();
        makeCurve();
    }
}
...
void Spline::makeSplit(int m_nSegs, xfl::enumDistribution distrib, QVector<double> &split) const
{
    if(m_nSegs<0) return;
...
{
    for(int i=0; i<m_CtrlPt.count(); i++)
    {
        m_CtrlPt[i] *= ratio;
    }
    updateSpline();
