...
void XflScriptExec::makeFoilAnalysisList()
{
    ...
    for(int ip=0; ip<m_oaRawFoil.size(); ip++)
    {
        ...
        for(int iPolar=0; iPolar<m_pScriptReader->m_Reynolds.size(); iPolar++)
        {
            ...
            pFoilAnalysis->vMin = m_pScriptReader->m_FoilAlphaMin;
            pFoilAnalysis->vMax = m_pScriptReader->m_FoilAlphaMax;
            pFoilAnalysis->vInc = m_pScriptReader->m_FoilAlphaInc;


            Objects2d::insertPolar(pPolar);
            m_FoilExecList.append(pFoilAnalysis);
            traceLog("   added analysis task for  ("+pFoil->name()+", "+pPolar->name()+")\n");
        }
        ...
        for(int ip=0; ip< m_pScriptReader->m_XmlFoilAnalysisList.size(); ip++)
        {
            ...
            if(bFound)
            {
                ...
                else
                {
                    ...
                    {
                        if(pFoil->name().compare(pPolar->foilName(), Qt::CaseInsensitive)==0)
                        {
                            pFoilAnalysis->pFoil = pFoil;
                            pFoilAnalysis->pPolar = pPolar;
    ...
    for(int ifo=0; ifo<m_pScriptReader->m_PolarFileList.count(); ifo++)
    {
        ...
        else
        {
            ...
            for (int ifoil=0;ifoil<foilList.size(); ifoil++)
            {
                ...
                if(pFoil)
                {
                    traceLog("   added foil and polars for: "+pFoil->name()+"\n");
            ...
            for (int ipolar=0; ipolar<polarList.size(); ipolar++)
            {
...
bool XflScriptExec::loadXFoilPolarFiles()
{
    ...
    for(int ifo=0; ifo<XFoilPolarList.count(); ifo++)
    {
        ...
        if(pPolar)
        {
            ...
            QString foilname = pPolar->foilName();
            ...
            Objects2d::insertPolar(pPolar);
            logmsg += ("   added the XFoil polar: "+ foilname +" / "+pPolar->name() +"\n");
        }
        else
        {
...
bool XflScriptExec::makeFoils()
{
    ...
    for(int ifo=0; ifo<m_pScriptReader->m_FoilDatList.count(); ifo++)
    {
        ...
        if (!datPathName.length())
        {
            QString strange = "   ...failed to find the file "+m_pScriptReader->m_FoilDatList.at(ifo);
            traceLog(strange+"\n");
            continue;
        }
        ...
        else
        {
            ...
            if(pFoil)
            {
                ...
                pFoil->setLineColor(xfl::randomColor());
            ...
            {
                traceLog("   ...failed to add the foil from "+m_pScriptReader->m_FoilDatList.at(ifo)+"\n");
                continue;
            }
        ...
        if(planereader.readFile())
        {
            ...
            if(checkPlaneFoils(pPlane))
            {
                ...
                for(int ip=0; ip<m_oaPlane.size(); ip++)
                {
                    ...
                    {
                        bExists = true;
                        traceLog("   The plane: "+pPlane->name() +" is defined multiple times... discarding redundant occurences\n");
                    }
                }
                if(!bExists)
                {
                    ...
                    traceLog("   added the plane: "+pPlane->name()+" from file "+fi.fileName()+"\n");
                }
            }
            else
            {
...
bool XflScriptExec::runScript(QString const &scriptpath)
{
    ...
    if(!setLogFile(logfilename)) return false;
...
//    traceLog("Checking foil availability...\n");
    ...
    for(int iw=0; iw<pPlane->nWings(); iw++)
    {
        ...
        if(pWing)
        {
            for(int iSec=0; iSec<pWing->sectionCount(); iSec++)
            {
                QString rightFoilName = pWing->m_Section[iSec].rightFoilName();
                ...
                {
                    QString leftFoilName = pWing->m_Section[iSec].leftFoilName();
    ...
    if(missingFoilList.size())
    {
        for(int iFoil=0; iFoil<missingFoilList.size(); iFoil++)
            traceLog("      ...missing foil "+missingFoilList.at(iFoil)+" for plane "+pPlane->name()+"\n");
    }
...
void XflScriptExec::startXFoilTaskThread()
{
    ...
    pXFoilTask->initializeTask(pAnalysis, false, true, true);
    if(pAnalysis->pPolar->isType12())
        pXFoilTask->setAlphaRange(pAnalysis->vMin, pAnalysis->vMax, pAnalysis->vInc);
//    else if(pAnalysis->pPolar->isFixedaoaPolar())        pXFoilTask->setReRange(pAnalysis->vMin, pAnalysis->vMax, pAnalysis->vInc);

    //launch it
    m_nTaskStarted++;
    strong = "Starting "+pAnalysis->pFoil->name()+" / "+pAnalysis->pPolar->name()+"\n";
    traceLog(strong);
...
void XflScriptExec::rePanelFoil(Foil *pFoil)
{
    ...
    traceLog("      done re-panelling foil: "+pFoil->name()+"\n");
}
...
//        QFileInfo fi(xmlFile);
        ...
        if(BoatReader.readXMLBoatFile() && BoatReader.boat())
        {
            ...
            traceLog("      The boat "+pBoat->name()+" has been read successfully\n");
        }
        else
        {
            QString strange = "      ...failed to load the file "+xmlFile.fileName()+"\n";
...
void XflScriptExec::makeBtPolarArray()
{
    ...
    for(int iwp=0; iwp<m_pScriptReader->m_BtPolarFileList.count(); iwp++)
    {
        ...
        if(pBtPolar)
        {
            traceLog("   the analysis file "+pBtPolar->name()+ " has been read successfully.\n");
            ...
            else
            {
                ...
                for(int ip=0; ip<m_oaBoat.size(); ip++)
                {
                    ...
                    traceLog("   the analysis "+pNewBtPolar->name()+ " has been added for the Boat "+ pBoat->name()+"\n");
                }
        ...
        if(pBoat)
        {
            for(int iwp=0; iwp<m_oaBtPolar.count(); iwp++)
            {
                ...
                if(pBtPolar->boatName().compare(pBoat->name())==0)
                {
                    ...
                    {
                        vMin = m_pScriptReader->m_T6Range.first().m_vMin;
                        vMax = m_pScriptReader->m_T6Range.first().m_vMax;
                        vInc = m_pScriptReader->m_T6Range.first().m_vInc;
                    }
                    ...
                    traceLog("   added analysis for ("+pBoat->name()+", "+pBtPolar->name()+")\n");
...
void XflScriptExec::runBoatAnalyses()
{
    ...
    for(int ia=0; ia<m_BoatExecList.size(); ia++)
    {
        ...
        pBoatTask->setEventDestination(this);

        connect(this, SIGNAL(cancelTask()), pBoatTask, SLOT(onCancel()),         Qt::QueuedConnection);

        strong = "\n   Launching Boat analysis: " + pBoatTask->boat()->name() + " / " + pBoatTask->btPolar()->name() + "\n";
        traceLog(strong);
...
void XflScriptExec::cleanUpBoatTask(BoatTask *pBoatTask)
{
    ...
    for(int iBtOpp=0; iBtOpp<pBoatTask->BtOppList().size(); iBtOpp++)
    {
