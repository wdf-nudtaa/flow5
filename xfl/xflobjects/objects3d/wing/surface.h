...
class Surface
{
    ...
    public:
        ...
        void getSidePoints_1(xfl::enumSurfacePosition pos, const Fuse *pFuse, QVector<Node> &PtA, QVector<Node> &PtB, int nPoints, xfl::enumDistribution disttype) const;
        void getSidePoints1_task(int nPoints, int tmp_nPoints, Node *nodeA, Node *nodeB) const;

        void getSidePoints_2(xfl::enumSurfacePosition pos, const Fuse *pFuse, QVector<Vector3d> &PtA, QVector<Vector3d> &PtB, QVector<Vector3d> &NA, QVector<Vector3d> &NB, QVector<double> const &xPointsA, QVector<double> const &xPointsB) const;
        ...
        bool hasPanel3(int idx3) const {return m_Panel3List.contains(idx3);}
        bool hasPanel4(int idx4) const {return m_Panel4List.contains(idx4);}

        inline bool hasFlapPanel3(const Panel3 &p3) const;
        bool hasFlapPanel3(int idx3)   const {return m_FlapPanel3.contains(idx3);}
        QVector<int> const & flapPanel3() const {return m_FlapPanel3;}


        inline bool hasFlapPanel4(Panel4 const &p4) const;
        bool hasFlapPanel4(int idx4)   const {return m_FlapPanel4.contains(idx4);}
        bool isFlapNode(int nNode)     const {return m_FlapNode4.contains(nNode);}
        ...
        void setFoilNames(QString const &nameA, QString const &nameB) {m_FoilNameA=nameA; m_FoilNameB=nameB;}

        QString const & leftFoilName() const {return m_FoilNameA;}
        QString const & rightFoilName() const {return m_FoilNameB;}
        ...
        int makeTipNodes(QVector<Node> &nodes, bool bLeft, int nStrips) const;
        void makePanelNodes(QVector<Node> &nodes, bool bMakeLeftNodes, bool bMidSurface) const;

        void makeTriPanels(QVector<Panel3> &panel3list, QVector<Node> &nodes, bool bThickSurfaces, int nTipStrips, int &leftnodeidx, int &rightnodeindex);
        int makeQuadPanels(QVector<Panel4> &panel4list, int &nWakeColumn, bool bThickSurface, int nTipStrips);

        int isNode(Vector3d const &Pt, QVector<Panel4> const & panel4list) const;
        QVector<int> const &panel3list() const {return m_Panel3List;}
        QVector<int> const &panel4List() const {return m_Panel4List;}

        inline Vector3d TEbisector(Vector3d const &Pt) const;

        QVector<double> xDistribA() const {return m_xPointA;}
        QVector<double> xDistribB() const {return m_xPointB;}
        void setXDistribA(QVector<double> const &leftdistrib)  {m_xPointA=leftdistrib;}
        void setXDistribB(QVector<double> const &rightdistrib) {m_xPointB=rightdistrib;}

        int nPanel3() const {return m_Panel3List.size();}
    ...
    private:
        QVector<Node> m_SideA;        /**< the array of panel points on the left foil's mid-line*/
        QVector<Node> m_SideB;        /**< the array of panel points on the right foil's mid-line*/
        QVector<Node> m_SideA_Top;    /**< the array of panel points on the left foil's top-line*/
        QVector<Node> m_SideB_Top;    /**< the array of panel points on the right foil's top-line*/
        QVector<Node> m_SideA_Bot;    /**< the array of panel points on the left foil's bottom-line*/
        QVector<Node> m_SideB_Bot;    /**< the array of panel points on the right foil's bottom-line*/
    ...
    private :
        ...
        QVector<int> m_FlapNode4;     /**< @todo remove the array of flap node indexes, used to avoid defining two nodes at the same location */
        QVector<int> m_FlapPanel4;   /**< the array of flap panel indexes */
        QVector<int> m_FlapPanel3;   /**< the array of flap panel indexes */
        ...
        QString m_FoilNameA;           /**< the name of the Surface's left foil name */
        QString m_FoilNameB;           /**< the name of the Surface's right foil name */

        QVector<int> m_Panel3List;    /**< the list of panel3 indexes of this surface */
        QVector<int> m_Panel4List;    /**< the list of panel4 indexes of this surface */
...
inline bool Surface::hasFlapPanel3(Panel3 const &p3) const {return m_FlapPanel3.contains(p3.index());}


inline bool Surface::hasFlapPanel4(const Panel4 &p4) const {return m_FlapPanel4.contains(p4.index());}
