...
QVector<Vector3d> Surface::s_DebugPts;
QVector<Vector3d> Surface::s_DebugVecs;
...
/*
Surface::Surface(Surface const &aSurface)
{
    copy(aSurface);
}*/
...
{
    //Add Nodes
    if(!m_FlapNode4.contains(p4.m_iLA)) m_FlapNode4.append(p4.m_iLA);
    if(!m_FlapNode4.contains(p4.m_iLB)) m_FlapNode4.append(p4.m_iLB);
    if(!m_FlapNode4.contains(p4.m_iTA)) m_FlapNode4.append(p4.m_iTA);
    if(!m_FlapNode4.contains(p4.m_iTB)) m_FlapNode4.append(p4.m_iTB);

    //Add panels
    if(!m_FlapPanel4.contains(p4.index())) m_FlapPanel4.append(p4.index());
}


void Surface::addFlapPanel3Index(int idx)
{
    if(!m_FlapPanel3.contains(idx))
        m_FlapPanel3.append(idx);
}


void Surface::addPanel3Index(int idx)
{
    if(!m_Panel3List.contains(idx))
        m_Panel3List.append(idx);
}


void Surface::addPanel4Index(int idx)
{
    if(!m_Panel4List.contains(idx))
        m_Panel4List.append(idx);
}
...
void Surface::copy(Surface const &aSurface)
{
    ...
    m_FoilNameA = QString(aSurface.m_FoilNameA);
    m_FoilNameB = QString(aSurface.m_FoilNameB);
 ...
 */
int Surface::makeTipNodes(QVector<Node> &nodes, bool bLeft, int nStrips) const
{
    Node node;
    QVector<Node> nodetiplist((m_NXPanels+1)*(nStrips+1));
    int n0 = nodes.size();
 ...
 * This is to avoid confusion of the trailing nodes at the surface's tip
 */
void Surface::makePanelNodes(QVector<Node> &nodes, bool bMakeLeftNodes, bool bMidSurface) const
{
    double y1=0, y2=0;
    Node node;
    QVector<Node> lefttiplist;
    int n0 = nodes.size();
...
void Surface::getSidePoints_1(xfl::enumSurfacePosition pos,
                              Fuse const *pFuse,
                              QVector<Node> &PtA, QVector<Node> &PtB,
                              int nPoints, xfl::enumDistribution ) const
{
    ...
    if(bMultithread)
    {
        ...
        for(int i=0; i<nPoints; i++)
        {
            ...
            futureSync.addFuture(QtConcurrent::run(this, &Surface::getSidePoints1_task, i, nPoints, &PtA[i], &PtB[i]));
#else
            futureSync.addFuture(QtConcurrent::run(&Surface::getSidePoints1_task, this, i, nPoints, &PtA[i], &PtB[i]));
#endif
        }
...
//Note: passing references to nodeA and node B is not compatible with multithreading for unknown reasons
...
void Surface::getSidePoints_2(xfl::enumSurfacePosition pos,
                              const Fuse *pFuse,
                              QVector<Vector3d> &PtA, QVector<Vector3d> &PtB, QVector<Vector3d> &NA, QVector<Vector3d> &NB,
                              QVector<double> const &xPointsA, QVector<double> const &xPointsB) const
{
    Q_ASSERT(xPointsA.size()==xPointsB.size());
...
//    auto t0 = std::chrono::high_resolution_clock::now();
    ...
    QVector<Node> nodeA(xPointsA.size());
    QVector<Node> nodeB(xPointsB.size());
    for(int i=0; i<xPointsA.size(); i++)
    {
...
void Surface::makeSideNodes(Fuse const*pTranslatedFuse, bool bDebug)
{
    ...
    if(bMultihread)
    {
        ...
        {
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
            futureSync.addFuture(QtConcurrent::run(this, &Surface::makeSideNodeTask, l, pTranslatedFuse, m_xPointA.at(l), m_xPointB.at(l)));
#else
    ...
    {
        s_DebugPts.clear();
        for(int i=0; i<m_SideA_Top.size(); i++) s_DebugPts.append(m_SideA_Top.at(i));
        for(int i=0; i<m_SideA_Top.size(); i++) s_DebugPts.append(m_SideB_Top.at(i));
    }
}
...
void Surface::makeTriPanels(QVector<Panel3> &panel3list, QVector<Node> &nodes, bool bThickSurfaces, int nTipStrips, int &leftnodeidx, int &rightnodeidx)
{
    ...
    // from bot, side and top surface
    int nRightTipRearNode = nodes.size()-1;
...
/*        s_DebugPts.clear();
        for(int i=0; i<m_SideA_Bot.size(); i++)  s_DebugPts.append(m_SideA_Bot.at(i));
        for(int i=m_SideA_Top.size()-2; i>=0; i--)  s_DebugPts.append(m_SideA_Top.at(i));
*/
        ...
        for(int is=0; is<nTipStrips; is++)
        {
            ...
            for (int l=0; l<m_NXPanels; l++)
            {
                ...
                if(!p3tup.isNullTriangle()) /** @todo any use? */
                {
                    ...
                    p3T.setIndex(panel3list.size()-1);
                    p3T.m_bIsLeftWingPanel  = true;
                    p3T.m_iPD = panel3list.size()-2;
                    p3T.m_iPU = (l==m_NXPanels-1) ? -1 : panel3list.size();
                ...
                if(!p3tlw.isNullTriangle())
                {
                    ...
                    p3B.setIndex(panel3list.size()-1);
                    p3B.m_bIsLeftWingPanel  = true;
                    p3B.m_iPD = panel3list.size()-2;
                    p3B.m_iPU = panel3list.size();
    ...
    m_FirstStripPanelIndex = panel3list.size();
    ...
    for (int k=0; k<m_NYPanels; k++)
    {
        ...
        for (int l=0; l<2*m_NXPanels; l++)
        {
            ...
            if(!p3Dtmp.isNullTriangle())
            {
                ...
                p3D.setIndex(panel3list.size()-1);
                ...
                p3D.m_iPD = panel3list.size()-2;
                p3D.m_iPU = panel3list.size();
            ...
            if(!p3Utmp.isNullTriangle())
            {
                ...
                p3U.setIndex(panel3list.size()-1);
                ...
                p3U.m_iPD = panel3list.size()-2;
                p3U.m_iPU = panel3list.size();
    ...
    if(bThickSurfaces)
        m_LastStripPanelIndex = panel3list.size()-2*2*m_NXPanels;
    else
        m_LastStripPanelIndex = panel3list.size()-  2*m_NXPanels;
...
int Surface::makeQuadPanels(QVector<Panel4> &panel4list, int &nWakeColumn, bool bThickSurfaces, int nTipStrips)
{
    int k=0;
    int n4_0 = panel4list.size();
    ...
    if (bLeftPatch)
    {
        ...
        for(int is=0; is<nTipStrips; is++)
        {
            ...
            for (int l=0; l<m_NXPanels; l++)
            {
                ...
                p4T.setIndex(panel4list.size()-1);
                p4T.m_bIsLeftWingPanel  = true;
                if(l>0)             p4T.m_iPD = panel4list.size()-2;
                if(l<m_NXPanels-1)  p4T.m_iPU = panel4list.size();
    ...
    int nb0 = panel4list.size(); // the index of the first bottom surface panel
...
//            s_DebugPts.append(p4.m_CtrlPt);
//            s_DebugVecs.append(p4.trailingVortex());
    ...
    if (bRightPatch)
    {
        ...
        int in = 0;
        int nsp = panel4list.size()-1; // the index of the last top surface panel
        for(int is=0; is<nTipStrips; is++)
        {
            in = is * nStripNodes;
            // make a strip
...
bool Surface::makeSectionSplines(BSpline3d &leftspline, BSpline3d &rightspline) const
{
    ...
    for(int i=PtA_B.size()-1; i>=0; i--) points.append(PtA_B.at(i));
    for(int i=0; i<PtA_T.size(); i++) points.append(PtA_T.at(i));
    if(!leftspline.approximate(3, nCtrlPoints, points)) return false;

    //Right spline
    points.clear();
    for(int i=PtB_B.size()-1; i>=0; i--) points.append(PtB_B.at(i));
    for(int i=1; i<PtB_T.size(); i++) points.append(PtB_T.at(i));
...
//    int nCtrlPoints = 11;
//    int nPoints = 2*nCtrlPoints; // minimum to get a good approximation
    QVector<Vector3d> PtA(nOutPoints), PtB(nOutPoints);
    QVector<Vector3d> NA(nOutPoints), NB(nOutPoints);
    QVector<double> xdistrib;
...
bool Surface::makeSectionSplinesOcc(bool bTop, bool bLeft, Handle(Geom_BSplineCurve)& theSpline) const
{
    int nCtrlPoints = 11;
    int nPoints = 2*nCtrlPoints; // minimum to get a good approximation
    QVector<Vector3d> PtA(nPoints),  PtB(nPoints);
    QVector<Vector3d> NA(nPoints), NB(nPoints);
    QVector<double> xdistrib;
