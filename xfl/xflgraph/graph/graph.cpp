...
Graph::Graph()
{
    ...
    m_theBorderStyle.m_Color = QColor(55,55,55);
...
QColor Graph::axisColor(AXIS::enumAxis axis) const
{
    switch (axis)
    {
        case AXIS::XAXIS:         return m_XAxis.colour();
        case AXIS::LEFTYAXIS:     return m_YAxis[0].colour();
        case AXIS::RIGHTYAXIS:    return m_YAxis[1].colour();
    }
    return Qt::gray;
}
...
void Graph::drawGraph(QPainter &painter, QRectF const &graphrect)
{
    ...
    //    Draw Border
    if(m_bBorder) color = m_theBorderStyle.m_Color;
...
void Graph::drawAxes(QPainter &painter) const
{
    ...
    QPen AxesPen;
    AxesPen.setColor(m_XAxis.colour());
...
void Graph::drawYAxis(int iy, QPainter &painter) const
{
    ...
    QPen AxesPen;
    AxesPen.setColor(m_YAxis[iy].colour());
...
void Graph::drawYGrid(int iy, QPainter &painter) const
{
    ...
    QPen labelpen(m_YAxis[iy].colour());
            ...
            if(pCurve->size()>0 && strong.length())//is there anything to draw?
            {
                ...
                if(m_pCurveModel->isCurveSelected(pCurve) && isHighLighting())
                {
                    if(DisplayOptions::isLightTheme())
                        linecolor = pCurve->color().darker();
                    else
                        linecolor = pCurve->color().lighter();
...
void Graph::saveSettings(QSettings &settings)
{
    settings.beginGroup(m_Name);
    {
        ...
        m_theBorderStyle.saveSettings(settings, "BorderStyle");
