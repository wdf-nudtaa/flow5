...
#include <xfl/opengl/gl_occ.h>
#include <xfl/opengl/gl_xfl.h>
...
#include <xflobjects/analysis3d/streamlinemaker.h>
...
void gl3dXPlaneView::glRenderPanelBasedBuffers()
{
    ...
    if(pPOpp && pPOpp->hasVortons())
    {
        ...
        {
            double coef = 0.1;
            for(int iv=0; iv<pPOpp->m_VortexNeg.size(); iv++)
            {
    ...
    if(m_bMeshPanels && pWPolar)
    {
        if(pPlane->isXflType() && pWPolar->isQuadMethod())
        {
            ...
            if(pWPolar->isVLM() && m_bVortices)
            {
                ...
                for(int i4=0; i4<pPlaneXfl->quadpanels().size(); i4++)
                {
...
void gl3dXPlaneView::glRenderPOppBasedBuffers()
{
    ...
    if(pPlane->isXflType())
    {
        ...
        {
            paintSegments(m_pglXPlaneBuffers->m_vboDW,        W3dPrefs::s_VelocityStyle);
        }
        if(m_pPOpp3dControls->m_bLiftStrip) paintSegments(m_pglXPlaneBuffers->m_vboStripLift, W3dPrefs::s_LiftStyle);
        if(m_pPOpp3dControls->m_bICd)       paintSegments(m_pglXPlaneBuffers->m_vboIndDrag,   W3dPrefs::s_IDragStyle);
        if(m_pPOpp3dControls->m_bVCd)       paintSegments(m_pglXPlaneBuffers->m_vboViscDrag,  W3dPrefs::s_VDragStyle);

        if(s_pXPlane->isCpView())
        {
            // node normal has Cp amplitude
            for(int i=0; i<m_CpSections.size(); i++)
        ...
        if(m_pPOpp3dControls->m_bPartForces)
        {
            ...
            Vector3d windD(windDirection(pPOpp->alpha(), pPOpp->beta()));
            Vector3d force(pPOpp->m_AF.Fff() + windD * pPOpp->m_AF.viscousDrag());
            paintThickArrow(pPOpp->m_AF.centreOfPressure(), force*qDyn*sc, W3dPrefs::s_LiftStyle.m_Color, m_matModel);
...
bool gl3dXPlaneView::glMakeStreamLines(QVector<Panel3> const&panel3list, QVector<Node>const &nodelist, PlaneOpp const *pPOpp)
{
    ...
    {
        for (int i3=0; i3<panel3list.count(); i3++)
        {
    ...
    //                VA*= s_pXPlane->m_pCurPlane->rootChord()/50.0;
                    ...
                    if(pPlaneXfl && W3dPrefs::s_bUseWingColour)
                    {
                        for(int iw=0; iw<pPlaneXfl->nWings(); iw++)
                        {
                            ...
                            {
                                m_StreamLineColours.append(pWing->color());
                                break;
                            }
                        }
                    }
...
bool gl3dXPlaneView::glMakeStreamLines(QVector<Panel4> const &panel4list, PlaneOpp const *pPOpp)
{
    if(m_pglXPlaneBuffers->m_vboStreamLines.isCreated()) m_pglXPlaneBuffers->m_vboStreamLines.destroy();
...
void gl3dXPlaneView::computeP4VelocityVectors(Opp3d const *pPOpp, QVector<Vector3d> const &points, QVector<Vector3d> &velvectors, bool bMultithread)
{
    ...
    else
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            makeQuadVelocityBlock(iBlock, points, pPOpp->gamma().constData(), pPOpp->sigma().constData(), velvectors.data());
        }
        update();
    }
}
...
void gl3dXPlaneView::computeP3VelocityVectors(Opp3d const *pPOpp, QVector<Vector3d> const&points,  QVector<Vector3d> &velvectors, bool bMultithread)
{
    ...
    else
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            makeTriVelocityBlock(iBlock, points, pPOpp->gamma().constData(), pPOpp->sigma().constData(), velvectors.data());
        }
        update();
    }
}
...
bool gl3dXPlaneView::intersectTheObject(Vector3d const &AA, Vector3d const &BB, Vector3d &I)
{
    ...
    if(m_pPOpp3dControls->m_bWakePanels && pPlane && pWPolar && pWPolar->isType6() && s_pXPlane->m_pCurPOpp)
    {
        ...
        {
            for(int i3=0; i3<pPlane->triMesh().wakePanels().size(); i3++)
            {
        ...
        {
            PlaneXfl const * pPlaneXfl = dynamic_cast<PlaneXfl const*>(pPlane);
            for(int i4=0; i4<pPlaneXfl->quadMesh().wakePanels().size(); i4++)
            {
    ...
    else
    {
        ...
        if(bPickNode())
        {
            ...
            else
            {
                ...
                {
                    Node const &nd = pPlane->node(m_PickedNodeIndex);
                    setTopRightOutput(nd.properties());
                 }
            }
        }
        else
...
bool gl3dXPlaneView::pickTriUniPanel(QPoint const &point)
{
    ...
    Plane const *pPlane = s_pXPlane->m_pCurPlane;
    WPolar const *pWPolar = s_pXPlane->m_pCurWPolar;
...
/*                if(pWPolar->isVLM1())
    ...
    if(m_pPOpp3dControls->m_bWakePanels && (s_bResetglMesh || s_bResetglWake))
    {
        if(pWPolar /* && pPOpp */)
        {
            ...
            {
                PlaneXfl const * pPlaneXfl = dynamic_cast<PlaneXfl const*>(pPlane);
                gl::makeQuadPanels(pPlaneXfl->quadMesh().wakePanels(), Vector3d(), m_pglXPlaneBuffers->m_vboWakePanels);
                gl::makeQuadEdges( pPlaneXfl->quadMesh().wakePanels(), Vector3d(), m_pglXPlaneBuffers->m_vboWakeEdges);
            }
            else if(pWPolar->isTriangleMethod())
            {
...
void gl3dXPlaneView::glMakeOppBuffers()
{
    ...
    if(s_bResetglPanelForce || s_bResetglOpp || s_bResetglMesh)
    {
        ...
        if (m_pPOpp3dControls->m_bPanelForce && pWPolar && pPOpp)
        {
            ...
            {
                QVector<Node> const &nodes = m_NodeVisible;
                TriMesh::makeNodeValues(pPlane->triMesh().nodes(), pPlane->triMesh().panels(),
        ...
        if(m_pPOpp3dControls->m_bPanelForce)
        {
            m_ColourLegend.setTitle(Units::pressureUnitLabel());
            m_ColourLegend.setRange(lmin, lmax);
            m_ColourLegend.makeLegend();
        }
    ...
    if(pPlane->isXflType())
    {
        ...
        if(m_pPOpp3dControls->m_bDownwash && (s_bResetglDownwash || s_bResetglOpp))
        {
            ...
            gl::makeArrows(points, arrows, s_VelocityScale, m_pglXPlaneBuffers->m_vboDW);
...
void gl3dXPlaneView::makeLLTDownwash(const PlaneXfl *pPlane, const WPolar *pWPolar, const PlaneOpp *pPOpp,
...
void gl3dXPlaneView::makeDownwash(const PlaneXfl *pPlane, const WPolar *pWPolar, const PlaneOpp *pPOpp,
                                  QVector<Vector3d> &points, QVector<Vector3d> &arrows) const
{
    ...
    QVector<Panel3> const &panel3 = pPlane->triPanels();
    QVector<Panel4> const &panel4 = pPlane->quadMesh().panels();
...
void gl3dXPlaneView::glMakeLLTTransitions(PlaneXfl const *pPlane, WPolar const *pWPolar, PlaneOpp const *pPOpp, QOpenGLBuffer &vbo) const
{
    if(!pPlane || !pPlane->mainWing() || !pWPolar || !pPOpp) return;
...
void gl3dXPlaneView::glMakeTransitions(const PlaneXfl *pPlane, const WPolar *pWPolar, const PlaneOpp *pPOpp, QOpenGLBuffer &vbo) const
{
    if(!pPlane || !pWPolar || !pPOpp) return;
    ...
    for(int iw=0; iw<pPlane->nWings(); iw++)
    {
        ...
        for(int j=0; j<pWing->m_Surface.size(); j++)
        {
            for(int k=0; k<pWing->m_Surface.at(j).NYPanels(); k++)
            {
                ...
                pWing->m_Surface.at(j).getSurfacePoint(SD.m_XTrTop.at(m), SD.m_XTrTop.at(m),yrel, xfl::TOPSURFACE, top, N);
                top.rotateY(pPOpp->alpha());

                yrel = pWing->ySectionRel(SD.m_StripPos.at(m));
                pWing->m_Surface.at(j).getSurfacePoint(SD.m_XTrBot.at(m), SD.m_XTrBot.at(m), yrel, xfl::BOTSURFACE, bot, N);
...
void gl3dXPlaneView::glMakeDragStrip(const PlaneXfl *pPlane, const WPolar *pWPolar, const PlaneOpp *pPOpp, bool bICd, bool bVCd)
{
    if(!pPlane || !pWPolar || !pPOpp)
    {
        m_pglXPlaneBuffers->m_vboIndDrag.destroy();
        m_pglXPlaneBuffers->m_vboViscDrag.destroy();
        return;
    }
    ...
    m_pglXPlaneBuffers->m_vboIndDrag.destroy();
    m_pglXPlaneBuffers->m_vboIndDrag.create();
    m_pglXPlaneBuffers->m_vboIndDrag.bind();
    m_pglXPlaneBuffers->m_vboIndDrag.allocate(pICdVertexArray.data(), ICDbuffersize * int(sizeof(GLfloat)));
    m_pglXPlaneBuffers->m_vboIndDrag.release();

    m_pglXPlaneBuffers->m_vboViscDrag.destroy();
    m_pglXPlaneBuffers->m_vboViscDrag.create();
    m_pglXPlaneBuffers->m_vboViscDrag.bind();
    m_pglXPlaneBuffers->m_vboViscDrag.allocate(pVCdVertexArray.data(), VCDbuffersize * int(sizeof(GLfloat)));
    m_pglXPlaneBuffers->m_vboViscDrag.release();
}


void gl3dXPlaneView::glMakeLLTDragStrip(const PlaneXfl *pPlane, const WPolar *pWPolar, const PlaneOpp *pPOpp, bool bICd, bool bVCd)
{
    if(!pPlane || !pPlane->mainWing() || !pWPolar || !pPOpp) return;
    ...
    m_pglXPlaneBuffers->m_vboIndDrag.create();
    m_pglXPlaneBuffers->m_vboIndDrag.bind();
    m_pglXPlaneBuffers->m_vboIndDrag.allocate(pICdVertexArray.data(), bufferSize * int(sizeof(GLfloat)));
    m_pglXPlaneBuffers->m_vboIndDrag.release();

    m_pglXPlaneBuffers->m_vboViscDrag.destroy();
    m_pglXPlaneBuffers->m_vboViscDrag.create();
    m_pglXPlaneBuffers->m_vboViscDrag.bind();
    m_pglXPlaneBuffers->m_vboViscDrag.allocate(pVCdVertexArray.data(), bufferSize * int(sizeof(GLfloat)));
    m_pglXPlaneBuffers->m_vboViscDrag.release();
}


void gl3dXPlaneView::glMakeMoments(double planformspan, WPolar const *pWPolar, PlaneOpp const *pPOpp, float reflength)
{
...
void gl3dXPlaneView::glMakeLiftStrip(PlaneXfl const*pPlane, WPolar const*pWPolar, PlaneOpp const*pPOpp)
{
    if(!pPlane || !pWPolar || !pPOpp)
    {
...
void gl3dXPlaneView::glMakeLLTLiftStrip(PlaneXfl const*pPlane, WPolar const*pWPolar, PlaneOpp const*pPOpp)
{
...
//            qDebug("ssboPanels %f MB", float(m_ssboPanels.size())/1024.0f/1024.0f);
        ...
        iv=0;
        for(int ir=0; ir<pPOpp->m_Vorton.size(); ir++)
        {
            for(int jc=0; jc<pPOpp->m_Vorton.at(ir).size(); jc++)
            {
...
//            qDebug("m_ssboVortons %f MB", float(m_ssboPanels.size())/1024.0f/1024.0f);
    ...
    if(m_bResetBoids)
    {
        ...
        QColor clr(W3dPrefs::s_FlowStyle.m_Color);
        QVector<float>BufferArray(buffersize);
        int iv=0;
...
void gl3dXPlaneView::moveBoids()
{
    ...
    m_shadFlow.bind();
    {
        ...
        m_shadFlow.setUniformValue(m_shadFlowLoc.m_UniColor,     W3dPrefs::s_FlowStyle.m_Color);
