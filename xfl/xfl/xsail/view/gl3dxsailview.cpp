...
gl3dXSailView::gl3dXSailView() : gl3dXflView()
{
    ...
    m_pP3UniAnalysis_ = new P3UniAnalysis;
    m_pP3LinAnalysis_ = new P3LinAnalysis;
...
//    if(m_vboWater.isCreated())          m_vboWater.destroy();
    ...
    if(m_pP3UniAnalysis_) delete m_pP3UniAnalysis_;
    if(m_pP3LinAnalysis_) delete m_pP3LinAnalysis_;
...
void gl3dXSailView::setLiveVortons(QVector<QVector<Vorton>> const &vortons)
{
...
//                    glMakeNurbsOutline(pFxfl->nurbs(), pFxfl->position(), m_vboBodyOutline[ifuse]);
            ...
            else if(pFuse->isOccType())
            {
                ...
                gl::makeTriangulation3Vtx(pFuse->triangulation(), pFuse->position(),
                                        m_vboFuseTriangulation[ifuse], false);
            }
    ...
    {
        int nSails = pBoat->sailCount();
        resizeSailBuffers(nSails);
        for(int isail=0; isail<pBoat->sailCount(); isail++)
        {
...
//            Vector3d position = pSail->position();
            ...
            else if(pSail->isOccSail())
            {
                ...
                glMakeShellOutline(pOccSail->shapes(), Vector3d(), m_vboSailOutline[isail]);
            }
            else if(pSail->isStlSail())
            {
                gl::makeTrianglesOutline(pSail->triangles(), Vector3d(), m_vboSailOutline[isail]);
            }
        }
    }
...
//    m_pvmMatrix = m_matProj * m_matView * m_matModel;
    ...
    for(int ihull=0; ihull<pBoat->nHulls(); ihull++)
    {
        ...
        if(pFuse->isVisible() && ihull<m_vboFuseTriangulation.size())
        {
            if(m_bSurfaces)
            {
                paintTriangles3Vtx(m_vboFuseTriangulation[ihull], pFuse->color(), false, isLightOn());
            }
            if(m_bOutline)
            {
...
void gl3dXSailView::glRenderOppBuffers()
{
    ...
    if(m_pDisplayCtrls->s_bPartForces && pBtOpp && pBtPolar)
    {
        ...
        paintThickArrow(pBtPolar->CoG(),
                        pBtOpp->m_AF.Fsum() *sc, W3dPrefs::s_LiftStyle.m_Color, m_matModel);
...
//            pSail->leadingEdgeAxis().listCoords(pSail->name());
    ...
    {
        double coef = 0.1;
        for(int iv=0; iv<pBtOpp->m_VortexNeg.size(); iv++)
        {
...
//            AWS.set(TWS.x-VB.x, TWS.y-VB.y, 0.0);
            ...
            if(Vb>0.01)
                paintThinArrow(O, AWS, W3dPrefs::s_WindStyle.m_Color.darker(), W3dPrefs::s_WindStyle.m_Width, Line::DOT, m_matModel);
            if(i==4)
            {
                x = (2.0*(O.x)+ AWS.x)/2.0;
                y = (2.0*(O.y)+ AWS.y)/2.0;
                ...
                glRenderText(x+0.015/double(m_glScalef), y+0.015/double(m_glScalef), z+0.015/double(m_glScalef),
                             "AWS" + INFCHAR + QString::asprintf("=%g ", AWS.norm()*Units::mstoUnit())+Units::speedUnitLabel(),
                             W3dPrefs::s_WindStyle.m_Color.darker(), true);
                ...
                if(Vb>0.01)
                {
                    ...
                    z=(2.0*(O.z)+ TWS.z)/2.0;
                    glRenderText(x+0.015/double(m_glScalef), y+0.015/double(m_glScalef), z+0.015/double(m_glScalef),
                                 "TWS"+INFCHAR+QString::asprintf("=%g ", TWS_inf.norm()*Units::mstoUnit())+Units::speedUnitLabel(),
                                 W3dPrefs::s_WindStyle.m_Color, true);
                    ...
                    glRenderText(x+0.015/double(m_glScalef), y+0.015/double(m_glScalef), z+0.015/double(m_glScalef),
                                 QString::asprintf("Boat speed=%g ", Vb*Units::mstoUnit())+Units::speedUnitLabel(),
                                 Qt::darkCyan, true);
                }
            }
        }
        ...
        paintLineStrip(m_vboTTWSpline, W3dPrefs::s_WindStyle.m_Color.darker(), W3dPrefs::s_WindStyle.m_Width, Line::DOT);
...
/*    if(pBtOpp)
    ...
    if(pBtOpp && (XSailDisplayCtrls::s_b3dCp || XSailDisplayCtrls::s_bGamma || XSailDisplayCtrls::s_bPanelForce))
    {
        ...
        for(int in=0; in<pBoat->triMesh().nNodes(); in++)
        {
            ...
            dist = float(distanceToLine3d(AA, BB, node));
...
void gl3dXSailView::paintForce(QOpenGLBuffer &vbo)
{
    ...
    m_shadLine.bind();
    {
        ...
        m_shadLine.setUniformValue(m_locLine.m_UniColor, W3dPrefs::s_LiftStyle.m_Color);
...
bool gl3dXSailView::glMakeStreamLines(QVector<Panel3> const &panel3list, Boat const *pBoat, const BoatOpp *pBtOpp)
{
    ...
    {
        for (int i3=0; i3<panel3list.size(); i3++)
        {
                ...
                if(!indexlist.contains(p3.nodeIndex(1)))
                {
                    ...
                    if(W3dPrefs::s_bUseWingColour)
                    {
                        for(int is=0; is<pBoat->nSails(); is++)
                        {
                            ...
                            {
                                m_StreamLineColours.append(pSail->color());
                                break;
                            }
                        }
                    }
    ...
    {
        m_pP3UniAnalysis_->initializeAnalysis(s_pXSail->curBtPolar(),0);
        m_pP3UniAnalysis_->setTriMesh(s_pXSail->curBoat()->triMesh());
    }
    else if(s_pXSail->curBtPolar()->isTriLinearMethod())
    {
        m_pP3LinAnalysis_->initializeAnalysis(s_pXSail->curBtPolar(),0);
        m_pP3LinAnalysis_->setTriMesh(s_pXSail->curBoat()->triMesh());
    }
    ...
    for (int in=0; in<ptList.size(); in++)
    {
        ...
        if     (s_pXSail->curBtPolar()->isTriUniformMethod()) pMaker->setP3Analysis(m_pP3UniAnalysis_);
        else if(s_pXSail->curBtPolar()->isTriLinearMethod())  pMaker->setP3Analysis(m_pP3LinAnalysis_);
...
void gl3dXSailView::computeP3VelocityVectors(Opp3d const *pBtOpp, QVector<Vector3d> const&points,  QVector<Vector3d> &velvectors)
{
    ...
    if(s_pXSail->curBtPolar()->isTriUniformMethod())
    {
        m_pP3UniAnalysis_->setTriMesh(s_pXSail->curBoat()->triMesh());
        m_pP3UniAnalysis_->initializeAnalysis(s_pXSail->curBtPolar(), 0);
        m_pP3UniAnalysis_->setVortons(pBtOpp->m_Vorton);
    }
    else if(s_pXSail->curBtPolar()->isTriLinearMethod())
    {
        m_pP3LinAnalysis_->setTriMesh(s_pXSail->curBoat()->triMesh());
        m_pP3LinAnalysis_->initializeAnalysis(s_pXSail->curBtPolar(), 0);
        m_pP3LinAnalysis_->setVortons(pBtOpp->m_Vorton);
    }
    ...
    if(xfl::isMultiThreaded())
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            ...
            futureSync.addFuture(QtConcurrent::run(this, &gl3dXSailView::makeTriVelocityBlock,
                                                       iBlock, points, pBtOpp->gamma().constData(), pBtOpp->sigma().constData(), velvectors.data()));
#else
    ...
    else
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            makeTriVelocityBlock(iBlock, points, pBtOpp->gamma().constData(), pBtOpp->sigma().constData(), velvectors.data());
        }
        update();
    }
}
...
void gl3dXSailView::makeTriVelocityBlock(int iBlock, QVector<Vector3d> const &C, double const *mu, double const *sigma, Vector3d *VField) const
{
    ...
    {
        if (s_pXSail->curBtPolar()->isTriUniformMethod())
            m_pP3UniAnalysis_->getVelocityVector(C.at(ip), mu, sigma, V, Vortex::coreRadius(), false, false);
        else if(s_pXSail->curBtPolar()->isTriLinearMethod())
            m_pP3LinAnalysis_->getVelocityVector(C.at(ip), mu, sigma, V, Vortex::coreRadius(), false, false);
...
bool gl3dXSailView::intersectTheObject(Vector3d const &AA, Vector3d const &BB, Vector3d &I)
{
    ...
    if(m_pDisplayCtrls->s_bWakePanels && pBtPolar)
    {
        if(pBtPolar->isTriangleMethod())
        {
            for(int i3=0; i3<pBoat->triMesh().wakePanels().size(); i3++)
            {
...
void gl3dXSailView::makeVisiblePanel3List(Boat const *pBoat, BoatPolar const*pBtPolar, QVector<Panel3> &panel3list)
{
    for(int i3=0; i3<pBoat->nPanel3(); i3++)
    {
        for(int isail=0; isail<pBoat->nSails(); isail++)
        {
            ...
            if(pSail && pSail->isVisible())
            {
                ...
                {
                    panel3list.append(pBoat->triMesh().panelAt(i3));
                }
            }
        }
...
/*        vec4 floats; // area, maxsize, sigma, gamma
        ...
        int NPanels = m_pP3UniAnalysis_->nPanels();
        int NWakePanels = m_pP3UniAnalysis_->nWakePanels();
        ...
        {
            Panel3 const &p3 = m_pP3UniAnalysis_->panel3At(i);
        ...
        for(int i=0; i<NPanels; i++)
        {
            ...
            if(p3.isTrailing())
            {
                ...
                Panel3 const *p3w = &m_pP3UniAnalysis_->wakePanelAt(p3.iWake());
                while(p3w)
                {
                    gammw[p3w->index()] += pBtOpp->gamma(3*p3.index())*sign;
                    // is there another wake panel downstream?
                    if(p3w->m_iPD>=0) p3w = &m_pP3UniAnalysis_->wakePanelAt(p3w->m_iPD);
                    else
                    {
                        p3w = nullptr;
                        break;
        ...
        {
            Panel3 const &p3 = m_pP3UniAnalysis_->wakePanelAt(iw);
        ...
        iv=0;
        for(int ir=0; ir<pBtOpp->m_Vorton.size(); ir++)
        {
            for(int jc=0; jc<pBtOpp->m_Vorton.at(ir).size(); jc++)
            {
...
void gl3dXSailView::moveBoids()
{
    ...
    BoatOpp   const *pBtOpp   = s_pXSail->curBtOpp();
    if(!pBoat || !pBtPolar || !pBtOpp) return;
    if(!pBtOpp->isTriUniformMethod()) return;

    int NPanels = m_pP3UniAnalysis_->nPanels();
    int NWakePanels = m_pP3UniAnalysis_->nWakePanels();
    Vector3d VInf = pBtOpp->windDir() * pBtOpp->QInf();
    QVector4D Qinf = QVector4D (VInf.xf(), VInf.yf(), VInf.zf(), 0.0f);
    ...
    m_shadFlow.bind();
    {
        ...
        m_shadFlow.setUniformValue(m_shadFlowLoc.m_HasGround,    1);
        m_shadFlow.setUniformValue(m_shadFlowLoc.m_GroundHeight, 0.0f);
