...
#include <xfl/globals/gui_params.h>
#include <xfl/globals/mainframe.h>
#include <xfl/globals/fileio.h>
#include <xfl/graphs/graphtilectrls.h>
#include <xfl/misc/aboutf5.h>
#include <xfl/misc/creditsdlg.h>
#include <xfl/options/prefsdlg.h>
#include <xfl/test/attractor2d.h>
...
#include <xfl/test/test3d/gl3dpanelfield.h>
#include <xfl/test/test3d/gl3dquadfield.h>
#include <xfl/test/test3d/gl3dsingularity.h>
#include <xfl/test/test3d/gl3dsurfaceplot.h>
#include <xfl/test/test3d/gl3dvortonfield.h>
#include <xfl/test/textoutputtestdlg.h>
...
#include <xfl3d/testgl/gl2dcomplex.h>
...
#include <xfl3d/testgl/gl3dquat.h>
...
#include <xflfoil/splinectrl/splinectrl.h>
...
#include <xflwidgets/customdlg/logmessagedlg.h>
#include <xflwidgets/customdlg/logmessagedlg.h>
...
#include <xflwidgets/view/section2doptions.h>
...
//void MainFrame::handleIOResults(xfl::enumApp iApp, bool bError);  // Qt6
void MainFrame::handleIOResults(int iApp, bool bError) // Qt5
{
    QApplication::setOverrideCursor(Qt::WaitCursor);
    if(iApp==xfl::XDIRECT)
    {
...
{
    QString strong;
    if(m_pXPlane->m_pCurPlane) strong = m_pXPlane->m_pCurPlane->name()+".fl5";
    ...
    if(pPlaneXfl)
    {
        for(int iw=0; iw<pPlaneXfl->nWings(); iw++)
        {
            ...
            for(int is=0; is<pWing->sectionCount(); is++)
            {
                ...
                if(pRFoil && !FoilList.contains(pRFoil)) FoilList.push_back(pRFoil);
                if(pLFoil && !FoilList.contains(pLFoil)) FoilList.push_back(pLFoil);
            }
        }
    }
...
void MainFrame::onSaveBoatAsProject()
{
    ...
    strong = pBoat->name().simplified()+"fl5";
    ...
    {
        WingSail const *pWS = dynamic_cast<WingSail const*>(pBoat->sailAt(is));
        if(pWS)
        {
            for(int is=0; is<pWS->sectionCount(); is++)
            {
                ...
                if(pFoil && !FoilList.contains(pFoil)) FoilList.push_back(pFoil);
            }
        }
    }
...
bool MainFrame::loadSettings()
{
    ...
    settings.beginGroup("MainFrame");
    {
        ...
        WPolar::setVariableNames(Units::lengthUnitLabel(), Units::speedUnitLabel(), Units::massUnitLabel(),
                                 Units::forceUnitLabel(), Units::momentUnitLabel());
        PlaneOpp::setVariableNames(Units::forceUnitLabel(), Units::momentUnitLabel());

        BoatPolar::setVariableNames(Units::speedUnitLabel(), Units::forceUnitLabel(), Units::momentUnitLabel());
...
bool MainFrame::exportAllPolars(QString const &pathname, xfl::enumTextFileType fileType) const
{
    ...
    for(int l=0; l<Objects2d::nPolars(); l++)
    {
        ...
        QString FoilSubDirPath = pathname + QDir::separator() + pFoil->name();
        ...
        if (XFile.open(QIODevice::WriteOnly | QIODevice::Text))
        {
            pPolar->exportPolar(out, xfl::versionName(true), false, bCSV);
            XFile.close();
        }
        else return false;
    }
...
bool MainFrame::exportAllWPolars(QString const &pathname, bool bCSV)
{
    ...
    for(int p=0; p<Objects3d::nPlanes(); p++)
    {
        ...
        if(pPlane && pPlane->isXflType())
        {
            ...
            QString PlaneSubDirPath = pathname + QDir::separator() + pPlaneXfl->name();
            ...
            {
                WPolar const *pWPolar = Objects3d::wPolarAt(l);
                if(!pWPolar) continue;
                if(pWPolar->planeName().compare(pPlaneXfl->name())!=0) continue;

                fileName = pWPolar->name();
                fileName.replace("/", "_");
                fileName.replace(".", "_");
...
bool MainFrame::exportAllBtPolars(QString const &pathname, bool bCSV)
{
    ...
    for(int p=0; p<SailObjects::boatCount(); p++)
    {
        Boat const *pBoat = SailObjects::boat(p);

        QString BoatSubDirPath = pathname + QDir::separator() + pBoat->name();
    ...
    for(int p=0; p<SailObjects::boatCount(); p++)
    {
        ...
        if(pBoat)
        {
            ...
            for(int l=0; l<SailObjects::btPolarCount(); l++)
            {
                ...
                if(!pBtPolar) continue;
                if(pBtPolar->boatName().compare(pBoat->name())!=0) continue;
                QString PolarName = pBtPolar->name();
                PolarName.replace("/", "_");
                PolarName.replace(".", "_");
                ...
                for(int k=0; k<SailObjects::btOppCount(); k++)
                {
                    BoatOpp const *pBtOpp = SailObjects::btOpp(k);
                    fileName = pBtOpp->title(false);
                    fileName.replace("/", "_");
                    fileName.replace(".", "_");
                    ...
                    if (XFile.open(QIODevice::WriteOnly | QIODevice::Text))
                    {
                        ...
                        pBtOpp->exportMainDataToString(pBoat, props, SaveOptions::exportFileType(), SaveOptions::textSeparator());
                        out << props;
                        if(bPanelData)
                        {
                            props.clear();
    ...
    for(int p=0; p<Objects3d::nPlanes(); p++)
    {
        ...
        if(pPlane && pPlane->isXflType())
        {
            ...
            for(int l=0; l<Objects3d::nPolars(); l++)
            {
                ...
                if(!pWPolar) continue;
                if(pWPolar->planeName().compare(pPlaneXfl->name())!=0) continue;
                QString PolarName = pWPolar->name();
                PolarName.replace("/", "_");
                PolarName.replace(".", "_");
                ...
                for(int k=0; k<Objects3d::nPOpps(); k++)
                {
                    ...
                    if (XFile.open(QIODevice::WriteOnly | QIODevice::Text))
                    {
                        ...
                        pPOpp->exportMainDataToString(pPlane, props, SaveOptions::exportFileType(), SaveOptions::textSeparator());
                        out << props;
                        if(bPanelData)
                        {
                            props.clear();
                            if(pPOpp->isQuadMethod())
                                pPOpp->exportPanel4DataToString(pPlaneXfl, pWPolar, SaveOptions::exportFileType(), props);
                            else if(pPOpp->isTriangleMethod())
                                pPOpp->exportPanel3DataToString(pPlaneXfl, pWPolar, SaveOptions::exportFileType(), SaveOptions::textSeparator(), props);
                            out <<props;
                        }
                        XFile.close();
                    }
...
bool MainFrame::exportAllStlMesh(QString const &pathname)
{
    for(int p=0; p<Objects3d::nPlanes(); p++)
    {
        ...
        if(pPlane && pPlane->isXflType())
        {
            ...
            QString fileName = pPlaneXfl->name()+".stl";
            fileName.replace("/", "_");
            ...
            {
                out.setDevice(&XFile);
                exportTriMesh(out, 1.0, pPlaneXfl->triMesh());
...
{
/*    IntValueDlg idlg(this);
    idlg.setLeftLabel("Test");
    idlg.setRightLabel("Right");
    idlg.setValue(100);
    idlg.exec();
    return;*/
