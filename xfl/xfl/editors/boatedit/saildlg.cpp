...
#include <xflwidgets/customdlg/helpimgdlg.h>
...
void SailDlg::makeCommonWts()
{
    ...
    m_pMeshBox = new QGroupBox("Mesh");
    {
        ...
        {
            QGroupBox *pgbMeshType = new QGroupBox("Mesh type");
            {
                ...
                pgbMeshType->setLayout(pMeshTypeLayout);
            }
            ...
            m_pgbEdgeSplit = new QGroupBox("Edge split", this);
            {
                QGridLayout *pEdgeLayout = new QGridLayout;
                {
                    ...
                    for(int iEdge=0; iEdge<4; iEdge++)
                    {
                        ...
                        m_pcbDistType[iEdge]->addItems(xfl::distributionTypes());
            ...
            pMeshLayout->addWidget(pgbMeshType);
    ...
    m_p2dViewFrame = new QFrame;
    {
        ...
        m_p2dViewFrame->setLayout(p2dViewLayout);
    }
...
void SailDlg::onSailColor()
{
    ...
    QColor clr = QColorDialog::getColor(m_pSail->color(), this, "Sail colour", QColorDialog::ShowAlphaChannel);
    if(clr.isValid())
    {
        m_pSail->setColor(clr);
        m_pcbColor->setColor(clr);
    }

    SailSectionView::sectionStyle().m_Color = clr;
    m_plbSectionStyle->setColor(clr);
...
void SailDlg::customEvent(QEvent *pEvent)
{
    if(pEvent->type() == MESH_UPDATE_EVENT)
    {
...
#ifdef QT_DEBUG
        if(pMeshEvent->SLG().size())
        {
            m_pglSailView->setSegments(pMeshEvent->SLG());
        }
        else m_pglSailView->clearSegments();
#endif
        ...
        if(AFMesher::s_DebugPts.size())
        {
            ...
            m_pglSailView->appendDebugVec(AFMesher::s_DebugPts.last()-AFMesher::s_DebugPts.first());
/*m_pSail->setRefTriangles(NURBSSurface::s_DbgTriangles);
m_pSail->makeTriPanels(Vector3d());*/
/*            Triangle3d const& t3d = NURBSSurface::s_DbgTriangles.at(5);
            Vector3d I;
            t3d.intersectSegmentInside(AFMesher::s_DebugPts.first(), AFMesher::s_DebugPts.last(), I, true);
...
void SailDlg::showEvent(QShowEvent *pEvent)
{
    ...
    if(m_pTabWidget)
        m_pTabWidget->setCurrentIndex(s_iActiveTab);
...
void SailDlg::hideEvent(QHideEvent *pEvent)
{
    ...
    if(m_pTabWidget) s_iActiveTab = m_pTabWidget->currentIndex();
}
...
{
    if(!m_pSail) return;
    m_pSail->setName(m_pleSailName->text());
    m_pSail->setPartDescription(m_pteSailDescription->toPlainText());
...
void SailDlg::loadSettings(QSettings &settings)
{
    settings.beginGroup("SailDlg");
    {
        ...
        SailSectionView::sectionStyle().loadSettings(settings, "SailSectionStyle");
...
void SailDlg::saveSettings(QSettings &settings)
{
    settings.beginGroup("SailDlg");
    {
        ...
        SailSectionView::sectionStyle().saveSettings(settings, "SailSectionStyle");
...
void SailDlg::onExportMeshToStl()
{
    ...
    exportMeshToSTLFile(pExportSail->triMesh(), m_pSail->name(), SaveOptions::STLDirName(), 1.0);
    delete pExportSail;
}
...
//        m_pFuse->exportStlTriangulation(out,1);
        exportTriangulation(out, 1.0, m_pSail->triangles());
    }
    else
    {
...
void SailDlg::onRuledMesh()
{
    ...
    m_pMesherWt->setVisible(!bRuled);
    m_pgbEdgeSplit->setVisible(!bRuled);

    m_pSail->setRuledMesh(bRuled);
    if(bRuled)
    {
        m_pptoOutput->onAppendThisPlainText("Making sail ruled mesh\n");
        m_pSail->makeRuledMesh(Vector3d()); // fast and automatic
        m_pptoOutput->onAppendThisPlainText(QString::asprintf("   made %d triangles\n\n", int(m_pSail->refTriangles().size())));
    }
    else
    {
...
//        Panel3 const &p3 = m_pSail->triMesh().panelAt(i3);
        ...
        {
            m_pptoOutput->onAppendThisPlainText("Connect the panels before attempting to guess the T.E.\n");
            return;
        }

        int idx = m_pSail->botMidTEIndexes().indexOf(i3);
        if(idx>=0) m_pSail->removeTEindex(i3, true); // user wants to deselect
        else
        {
            if(m_pSail->isThinSurface())
            {
                ...
                {
                    m_pptoOutput->onAppendThisPlainText("A thin T.E. panel must have one and only one free edge.\n");
                    return;
                }
            ...
            if(s_bGuessOpposite)
            {
                for(int i3=0; i3<t3d.neighbourCount(); i3++)
                {
                    ...
                    if(idx>=0 && idx<m_pSail->triMesh().nPanels())
                    {
                        ...
                        if(cos<ccrit)
                        {
                            int idx = m_pSail->topTEIndexes().indexOf(i3o);
                            if(idx<0) m_pSail->addTEindex(i3o, false);
                            m_pptoOutput->onAppendThisPlainText(QString::asprintf("Added opposite top side panel %d\n", i3o));
                        }
                    }
                }
            }
    ...
    else if(m_ppbTETop->isChecked())
    {
        ...
        int idx = m_pSail->topTEIndexes().indexOf(i3);
        if(idx>=0) m_pSail->removeTEindex(i3, false); // user wants to deselect
        else
        {
            ...
            if(s_bGuessOpposite)
            {
                for(int i3=0; i3<p3.neighbourCount(); i3++)
                {
                    ...
                    if(idx>=0 && idx<m_pSail->triMesh().nPanels())
                    {
                        ...
                        if(cos<ccrit)
                        {
                            int idx = m_pSail->botMidTEIndexes().indexOf(i3o);
                            if(idx<0) m_pSail->addTEindex(i3o, true);
                            m_pptoOutput->onAppendThisPlainText(QString::asprintf("Added opposite bottom side panel %d\n", i3o));
                        }
                    }
                }
            }
...
void SailDlg::onTopTEPanels(bool bChecked)
{
    ...
    {
        m_pptoOutput->onAppendThisPlainText("Make the mesh before setting the T.E.\n");
        return;
    }
...
void SailDlg::onConnectPanels()
{
    ...
    QApplication::setOverrideCursor(Qt::WaitCursor);
    m_pptoOutput->onAppendThisPlainText("Connecting panels...");
    ...
    Q_ASSERT(m_pSail->refTriangles().size()==m_pSail->nPanel3());
    m_pSail->saveConnections();

    QString log(" done\n\n");
    m_pptoOutput->onAppendThisPlainText(log);
    QApplication::restoreOverrideCursor();
}
...
void SailDlg::onCheckFreeEdges()
{
    ...
    m_pSail->triMesh().getFreeEdges(m_FreeEdges);
    m_pglSailView->setSegments(m_FreeEdges);
...
void SailDlg::onCheckTEPanels()
{
    ...
    if(bNoError)
    {
        m_pptoOutput->onAppendThisPlainText("No TE error.\n");
    }
    m_pglSailView->setHighlightList(errorlist);
    m_pglSailView->resetgl3dMesh();
    m_pglSailView->update();
...
void SailDlg::onGuessTE()
{
    ...
    if(m_pSail->nPanel3()<=0)
    {
        m_pptoOutput->onAppendThisPlainText("Build the mesh before setting the T.E.\n");
        return;
    }
    ...
    {
        if(guessThinTE())
            m_pptoOutput->onAppendThisPlainText(QString::asprintf("Found %d TE panels.\n", int(m_pSail->botMidTEIndexes().size())));
    }
    else
    {
        if(guessThickTE())
            m_pptoOutput->onAppendThisPlainText(QString::asprintf("Found %d pairs of TE panels.\n", int(m_pSail->topTEIndexes().size())));
    }
...
bool SailDlg::guessThinTE()
{
    ...
    bool bRes = makeFreeTELine(m_pSail->clew(), m_pSail->peak(), m_FreeEdges, TELine);
    if(!bRes)
    {
    ...
    QVector<Triangle3d> &triangles = m_pSail->m_RefTriangles;

    for(int it3=0; it3<triangles.size(); it3++)
    {
        ...
        for(int il=0; il<TELine.size(); il++)
        {
            ...
            if(iEdge>=0)
            {
                m_pSail->m_BotMidTEIndexes.append(it3);
...
void SailDlg::makeEdgeNodes(QVector<Node> &)
{
}
...
void SailDlg::onExportToStep()
{
    ...
    dlg.init(shape, m_pSail->name());
    dlg.exec();
}


void SailDlg::onReadEdgeSplits()
{
    QVector<EdgeSplit> &es = m_pSail->m_EdgeSplit.first();
